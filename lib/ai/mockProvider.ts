/**
 * AI Mock Provider — 完整实现
 *
 * 从 lib/aiMock.ts 迁移所有业务逻辑到此文件。
 * 所有函数签名与 IAIProvider 接口保持一致，
 * 未来切换到真实 AI 时只需更改 provider 配置，无需改业务代码。
 *
 * 设计约束：
 *   - 不依赖任何真实 API Key 或 SDK
 *   - 所有返回结构包裹在 AIResult<T> 中
 *   - 每个输出都附加 disclaimer + safetyNote
 *   - 模拟网络延迟（200-600ms）以贴近真实体验
 */
...
export function simulateThinkTime(): Promise<void> {
  return new Promise((r) => setTimeout(r, randInt(200, 600)))
}
...
export async function recommendCandidates(
  ...
  const key = industry === "both" ? "wind" : industry
  const recs = (MOCK_RECOMMENDATIONS[key] || []).map((r) => {
    let bonus = 0
    for (const req of requirements) {
      const lower = req.toLowerCase()
      if (r.highlights.some((h) => h.toLowerCase().includes(lower.slice(0, 5)))) bonus += 3
      if (r.reason.toLowerCase().includes(lower.slice(0, 5))) bonus += 2
    }
  ...
  return wrapAI({
    ...
    marketInsight: insights[key] || insights.wind,
    searchKeywords:
      key === "wind"
...
**Why You:**
Your track record in this field and your contributions demonstrate exactly the kind of expertise we value.
...
**为什么找你：**
你的履历在这个方向上属于 top 5%，尤其是关键技术/项目这块，市场上能做的人不多。
...
export async function generateInvitation(
  ...
  await simulateThinkTime()
  const generator = INVITATION_TEMPLATES[style] || INVITATION_TEMPLATES.formal
...
function generateWindQuestions(): InterviewQuestionSet["questions"] {
  return [
    { id: "q_w_1", category: "空气动力学", question: "请解释风力机叶片在低风速和高风速工况下的气动特性差异，以及您如何通过叶片设计优化功率曲线？", expectedFocus: "Betz 极限、翼型选型、失速控制、涡流发生器应用", difficulty: "advanced", domain: "wind" },
    { id: "q_w_2", category: "结构力学", question: "在海上风电叶片设计中，您如何平衡轻量化需求与极限载荷下的结构安全性？请结合实际项目经验。", expectedFocus: "碳纤维/玻纤复合材料、疲劳寿命、极限载荷工况、DNV 认证", difficulty: "advanced", domain: "wind" },
    { id: "q_w_3", category: "控制系统", question: "描述一个您处理过的风机控制系统故障案例，您是如何诊断和解决的？", expectedFocus: "变桨/偏航控制、PLC 编程、SCADA 数据分析、故障树分析", difficulty: "intermediate", domain: "wind" },
    { id: "q_w_4", category: "数字孪生", question: "您认为数字孪生在风电场运维中的最大价值是什么？请描述您构建数字孪生模型的方法论。", expectedFocus: "物理模型 vs 数据驱动、模型校准、预测性维护 ROI", difficulty: "intermediate", domain: "wind" },
    { id: "q_w_5", category: "行业认知", question: "您如何看待未来 5 年漂浮式海上风电的技术挑战和商业化前景？", expectedFocus: "浮式基础、动态电缆、运维策略、度电成本", difficulty: "basic", domain: "wind" },
  ]
}

function generateLithiumQuestions(): InterviewQuestionSet["questions"] {
  return [
    { id: "q_l_1", category: "电池材料", question: "请比较三元正极和磷酸铁锂正极在安全性、能量密度、循环寿命方面的差异，您倾向哪种技术路线？为什么？", expectedFocus: "NCM/NCA vs LFP、热失控机理、包覆改性策略", difficulty: "intermediate", domain: "lithium" },
    { id: "q_l_2", category: "电芯设计", question: "在大圆柱电池设计中，全极耳（tabless）结构相比传统多极耳有什么优势？设计难点是什么？", expectedFocus: "电流密度分布、热管理、制造工艺、内阻优化", difficulty: "advanced", domain: "lithium" },
    { id: "q_l_3", category: "BMS 算法", question: "描述一种您实现过的 SOC 估计算法（卡尔曼滤波/神经网络/等效电路模型），如何验证其精度？", expectedFocus: "EKF/AEKF/DKF、OCV-SOC 曲线、HPPC 测试、老化补偿", difficulty: "advanced", domain: "lithium" },
    { id: "q_l_4", category: "制造工艺", question: "在锂电制造中，涂布工序的哪些参数对电芯一致性影响最大？您如何处理涂布缺陷？", expectedFocus: "面密度控制、浆料流变性、干燥温度曲线、在线检测", difficulty: "intermediate", domain: "lithium" },
    { id: "q_l_5", category: "行业认知", question: "您认为钠离子电池在未来储能市场中的应用前景如何？与锂电相比，优劣势各是什么？", expectedFocus: "资源禀赋、能量密度、低温性能、产业成熟度", difficulty: "basic", domain: "lithium" },
  ]
}

function generateManagementQuestions(): InterviewQuestionSet["questions"] {
  return [
    { id: "q_m_1", category: "团队管理", question: "请分享一个您带领跨学科团队（如机械+电气+软件）完成复杂项目的案例。您如何协调不同背景的团队成员？", expectedFocus: "项目规划、冲突解决、资源分配、跨部门沟通", difficulty: "intermediate", domain: "management" },
    { id: "q_m_2", category: "技术决策", question: "当团队面临两种技术方案选择（自研 vs 外购），您的决策框架是什么？请举例说明。", expectedFocus: "成本分析、技术风险、长期战略、供应链依赖", difficulty: "intermediate", domain: "management" },
    { id: "q_m_3", category: "人才培养", question: "您如何识别和培养团队中的高潜人才？能否分享一个成功培养案例？", expectedFocus: "导师机制、技能矩阵、轮岗计划、反馈文化", difficulty: "basic", domain: "management" },
  ]
}
...
export async function generateInterviewQuestions(
  ...
  let questions: InterviewQuestionSet["questions"] = []

  if (domain === "general" || domain === "technical") {
    if (industry === "wind" || industry === "both") questions = questions.concat(generateWindQuestions())
    if (industry === "lithium" || industry === "both") questions = questions.concat(generateLithiumQuestions())
  }
  if (domain === "wind") questions = generateWindQuestions()
  if (domain === "lithium") questions = generateLithiumQuestions()
  if (domain === "management") questions = generateManagementQuestions()

  if (questions.length === 0) {
    questions = [
      { id: "q_g_1", category: "技术基础", question: "请介绍您最擅长的技术领域，以及您在该领域的核心能力。", expectedFocus: "技术深度、项目经验、学习能力", difficulty: "basic", domain: "general" },
      { id: "q_g_2", category: "项目经验", question: "请分享一个您主导的最具挑战性的技术项目，您是如何克服难点的？", expectedFocus: "问题解决能力、创新思维、团队协作", difficulty: "intermediate", domain: "general" },
    ]
  }
...
export async function generateAssessment(
  ...
  return wrapAI({
    ...
    summary: `${candidateName}（${candidateTitle}）整体面试表现 ${overall >= 80 ? "优秀" : "良好"}，在技术能力和行业认知方面表现突出，是 ${jobTitle} 岗位的${overall >= 85 ? "首选" : "候选"}人选。建议${overall >= 80 ? "尽快推进 Offer 流程" : "结合其他候选人综合评估"}。`,
    recommendation,
    recommendationText,
  })
}
...
const CURRENCY_MAP: Record<string, CurrencyCode> = {
  中国: "CNY", 丹麦: "EUR", 德国: "EUR", 美国: "USD", 新加坡: "SGD", 韩国: "USD", 日本: "JPY",
}
...
export async function suggestSalary(
  ...
  const baseline = SALARY_BASELINES[industry] || SALARY_BASELINES.wind
  const countryData = baseline[country] || baseline["中国"]
  ...
  const baseMin = Math.round(countryData.baseMin * expMultiplier)
  const baseMax = Math.round(countryData.baseMax * expMultiplier)
  ...
  return wrapAI({
    ...
    marketNote: `以上数据基于 ${country} 市场 ${industry === "wind" ? "风电" : "锂电"} 行业 ${experienceYears} 年经验工程师的历史薪酬分布，仅供参考。实际薪酬会因公司体量、候选人具体技能组合、谈判情况等因素显著变化。`,
...
export async function checkRisks(context: {
  ...
  const alerts: RiskCheckResult["alerts"] = []
  const trust = context.dataTrustLevel ?? 70

  if (trust < 40) {
    alerts.push({ id: "risk_trust_low", type: "data_trust", level: "critical", title: "数据可信度过低", description: `候选人数据可信度仅为 ${trust}%，数据源可能存在信息缺失或未验证情况。`, suggestion: "建议通过 LinkedIn 或直接联系候选人确认关键信息后再做决策。", relatedEntityType: "candidate" })
  } else if (trust < 60) {
    alerts.push({ id: "risk_trust_medium", type: "data_trust", level: "medium", title: "数据可信度偏低", description: `候选人数据可信度为 ${trust}%，部分信息未经验证。`, suggestion: "建议在面试中重点核实教育背景和工作经历。" })
  }

  if (context.authorizationStatus === "expired" || context.authorizationStatus === "revoked") {
    alerts.push({ id: "risk_auth_bad", type: "authorization", level: "critical", title: "授权状态异常", description: `候选人数据授权状态为「${context.authorizationStatus}」，继续使用其数据可能违反隐私条款。`, suggestion: "请立即暂停该候选人的数据处理，并联系法务团队确认合规方案。" })
  } else if (!context.authorizationStatus || context.authorizationStatus === "unknown") {
    alerts.push({ id: "risk_auth_unknown", type: "authorization", level: "high", title: "授权状态未知", description: "该候选人数据的授权状态无法确认，存在合规风险。", suggestion: "建议在 72 小时内确认候选人的数据使用授权状态。" })
  }

  if (context.candidateActivity === "low") {
    alerts.push({ id: "risk_response_low", type: "engagement", level: "high", title: "候选人回复率低", description: "该候选人在过去 30 天内对平台邀请的回复率低于 20%，可能已不活跃或不感兴趣。", suggestion: "考虑通过多渠道（邮件+LinkedIn+电话）尝试联系，或转向备选候选人。" })
  }

  const daysSinceInterview = context.interviewDaysSinceRequest ?? -1
  if (daysSinceInterview > 7) {
    alerts.push({ id: "risk_interview_overdue", type: "process", level: "high", title: "面试超时未反馈", description: `面试请求已发送 ${daysSinceInterview} 天，双方均未确认或反馈，流程可能停滞。`, suggestion: "建议 HR 立即跟进候选人，或与面试官确认安排。超过 14 天未反馈建议自动撤销请求。" })
  } else if (daysSinceInterview > 3) {
    alerts.push({ id: "risk_interview_pending", type: "process", level: "medium", title: "面试反馈待跟进", description: `面试请求已发出 ${daysSinceInterview} 天，建议在 48 小时内跟进。`, suggestion: "发送友好提醒给候选人和面试官确认时间安排。" })
  }

  const daysSinceOffer = context.offerDaysSinceSent ?? -1
  if (daysSinceOffer > 5) {
    alerts.push({ id: "risk_offer_overdue", type: "process", level: "critical", title: "Offer 审批/回复超时", description: `Offer 已发出 ${daysSinceOffer} 天未收到候选人确认或拒绝，存在候选人流失或被竞对截胡的风险。`, suggestion: "建议立即电话联系候选人确认意向，必要时设置 Offer 截止日期。" })
  }

  if (context.alreadyInvited) {
    alerts.push({ id: "risk_duplicate", type: "operation", level: "medium", title: "重复邀请风险", description: "该候选人已在 90 天内收到过同一岗位或类似岗位的邀请，重复发送可能影响品牌形象。", suggestion: "确认候选人之前拒绝的原因是否已变化，如无变化建议转向其他候选人。" })
  }

  if (context.candidateCountry && ["德国", "法国", "意大利", "西班牙", "荷兰"].includes(context.candidateCountry)) {
    alerts.push({ id: "risk_gdpr", type: "compliance", level: "low", title: "GDPR 合规提示", description: `候选人位于 ${context.candidateCountry}（欧盟/EEA），其数据处理受 GDPR 约束。请确保数据采集、存储和处理合规。`, suggestion: "确认候选人已签署 GDPR 同意书，数据存储在欧洲境内或等效合规区域。" })
  }
...
export async function generateCandidateSummary(
  ...
  const oneLinePitches: Record<string, string> = {
    wind: `${candidateName}，${candidateCountry}风能专家，${experienceYears}年经验，擅长叶片设计与风场数字化。`,
    lithium: `${candidateName}，${candidateCountry}锂电专家，${experienceYears}年经验，深耕${candidateTitle}方向。`,
    both: `${candidateName}，风储协同专家，${experienceYears}年跨领域经验，兼具技术研发与工程管理。`,
  }

  const key = candidateTitle.toLowerCase().includes("锂") || candidateTitle.toLowerCase().includes("电池") ? "lithium" : "wind"

  return wrapAI({
    candidateId,
    candidateName,
    oneLinePitch: oneLinePitches[key] || oneLinePitches.wind,
    keyStrengths: [
      `在${candidateTitle}方向有 ${experienceYears} 年实践经验`,
      "拥有可验证的项目成果或学术论文/专利",
      "跨文化/跨国项目协作经验",
    ],
    potentialConcerns: [
      experienceYears < 3 ? "经验年限偏短，需评估独立工作能力" : "",
      "需进一步核实推荐人反馈",
    ].filter(Boolean),
    suggestedRoles: [
      candidateTitle,
      `Senior ${candidateTitle}`,
      `Lead ${candidateTitle}`,
    ],
    recommendedNextAction: "建议安排 30 分钟技术电话面试，重点核实项目经历中的实际贡献。",
  })
}
